# Implicitly CXX := g++
# Modify this if you need include files from somewhere else.
MY_INCLUDES := $(HOME)/koodi/cpp/lib/include
CXXFLAGS := -march=native -O2 -std=c++20 -pedantic -pedantic-errors -Wextra -Wall -MMD -iquote $(MY_INCLUDES)
LDFLAGS :=
BUILD_DIR := ./build
# Add wanted binary names and their deps here (%.o added automatically for every %)
# Example
# MAINS := guess
# guess_deps := feikki.o
# TESTS := test_guess
MAINS := 01a 01b 02 03 04 05 06 07 09 10 11 12 13 14
TESTS :=
TARGETS := $(MAINS) $(TESTS)
BUILD_TARGETS := $(TARGETS:%=$(BUILD_DIR)/%)
SRC := $(wildcard src/*.cpp)
OBJ := $(SRC:src/%.cpp=$(BUILD_DIR)/%.o)
DEP := $(OBJ:%.o=%.d)

.PHONY: all

all: $(BUILD_TARGETS)

-include $(DEP)

$(BUILD_DIR)/%.o: src/%.cpp
	@mkdir -p $(BUILD_DIR)
	$(CXX) -c $< $(CXXFLAGS) -o $@

define TARGETS_template =
$(1): $$(BUILD_DIR)/$(1)
$$(BUILD_DIR)/$(1): $$(BUILD_DIR)/$(1).o $$(addprefix $$(BUILD_DIR)/,$$($(1)_deps))
endef

$(foreach main,$(TARGETS),$(eval $(call TARGETS_template,$(main))))

$(BUILD_TARGETS):
	$(LINK.cc) -o $@ $^

.PHONY: test
test: $(TESTS)
	$(foreach test,$^,$(BUILD_DIR)/$(test))

tags: $(wildcard src/*)
	ctags $^

.PHONY: clean
clean:
	rm -rf $(BUILD_DIR)
